...
Personal LLM evaluation harness. Run the same set of prompts against different models, auto-score responses via LLM-as-judge (1-5), and compare over time. Results persist as per-model JSON files in `results/`.
...
Single-entrypoint CLI (`run.py`) with three supporting modules:

- **`run.py`** - CLI dispatcher with argparse subcommands (`eval`, `compare`, `models`, `prompts`, `dashboard`). Also contains the data layer (load/save model results as JSON, filter prompts).
...
- **`scripts/dashboard.py`** - Generates a self-contained HTML dashboard (`dashboard.html`) with Chart.js visualizations. Shows leaderboard, category breakdowns, score distributions, latency comparisons, and auto-check flags. Auto-regenerated after each eval run.
